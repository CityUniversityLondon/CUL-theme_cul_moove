...
if ($confirm && isloggedin() && confirm_sesskey()) {
    ...
    show_course($cid);
    redirect($returnurl, get_string('courseshown', 'theme_cul_moove'));
}

// Otherwise, show a confirmation page.
$params = ['cid' => $cid, 'sesskey' => sesskey(), 'confirm' => 1];
$actionurl = new moodle_url('/theme/cul_moove/unhide_post.php', $params);

$PAGE->set_context($coursecontext);
$PAGE->set_url($actionurl);

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('showcourse', 'theme_cul_moove'));
echo $OUTPUT->box_start('generalbox', 'notice');

echo html_writer::tag('p', get_string('confirmshowcourse', 'theme_cul_moove'));
echo $OUTPUT->single_button($actionurl, get_string('showcourse', 'theme_cul_moove'), 'post');
